...
test.describe('host (nx-react)', () => {
  ...
  test('/remote1 loads the federated Counter', async ({ page }) => {
    await page.goto('/remote1', { waitUntil: 'networkidle' });
  ...
  test('/remote2 loads the federated Form', async ({ page }) => {
    await page.goto('/remote2', { waitUntil: 'networkidle' });
  ...
  test('/remote3 loads the federated List', async ({ page }) => {
    await page.goto('/remote3', { waitUntil: 'networkidle' });
...
test.describe('standalone remotes (nx-react)', () => {
  for (const { port, name, file } of [
    { port: 4201, name: 'remote1 standalone', file: '05-standalone-remote-1' },
    { port: 4202, name: 'remote2 standalone', file: '06-standalone-remote-2' },
    { port: 4203, name: 'remote3 standalone', file: '07-standalone-remote-3' },
  ]) {
