...
test.describe('host (rsbuild)', () => {
  ...
  test('/remote-1 loads the federated Counter', async ({ page }) => {
    ...
    await expect(page.getByTestId('remote-1')).toBeVisible();
  ...
  test('/remote-2 loads the federated Form', async ({ page }) => {
    ...
    await expect(page.getByTestId('remote-2')).toBeVisible();
  ...
  test('/remote-3 loads the federated List', async ({ page }) => {
    ...
    await expect(page.getByTestId('remote-3')).toBeVisible();
    await expect(page.getByTestId('user-list')).toBeVisible({ timeout: 15_000 });
    await expect(page.getByTestId('user-list').locator('li').first()).toBeVisible();
    await shoot(page, '04-remote-3-list');
  });
});

test.describe('standalone remotes (rsbuild)', () => {
  for (const { port, name, file } of [
    { port: 3001, name: 'remote-1 standalone', file: '05-standalone-remote-1' },
    { port: 3002, name: 'remote-2 standalone', file: '06-standalone-remote-2' },
    { port: 3003, name: 'remote-3 standalone', file: '07-standalone-remote-3' },
  ]) {
